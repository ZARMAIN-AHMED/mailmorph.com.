...
//     setTimeout(() => fetchUser(), 500); // ensure cookie is loaded
//   }, []);
...
//   // ✅ Login redirect
//   const handleLogin = () => {
//     window.location.href = `${API_BASE}/auth/login`; // redirect to backend login
//   };
...
//         setUser(null);
//         localStorage.removeItem("user");
//         setTimeout(() => fetchUser(), 500); // refresh user state
//       }
//     } catch (err) {
//       console.error("Logout failed:", err);
//     }
...
//     formData.append("name", tempName);
//     formData.append("bio", tempBio);
//     if (tempPic) formData.append("profilePic", tempPic);

//     try {
//       const res = await fetch(`${API_BASE}/user/update`, { method: "PATCH", body: formData, credentials: "include" });
//       if (!res.ok) return console.error("Profile update failed");
//       const data = await res.json();
//       setUser(data.user);
//       localStorage.setItem("user", JSON.stringify(data.user));
//       setEditMode(false);
...
//         {/* Logo */}
//         <a href="/" className="flex items-center gap-3 group">
//           <Image src="/logo2.png" alt="MailMorph" width={55} height={55} priority className="rounded-full" unoptimized />
//           <span className="text-2xl font-extrabold text-white group-hover:text-cyan-400">MailMorph</span>
//         </a>
...
//           {menuItems.map((item) => (
//             <li key={item.name}>
//               <a href={item.href} className="text-gray-200 hover:text-white font-medium">{item.name}</a>
//             </li>
//           ))}
//         </ul>
...
//           {user ? (
//             <>
//               <button onClick={() => setDropdownOpen(!dropdownOpen)} className="w-10 h-10 rounded-full overflow-hidden border-2 border-cyan-400">
//                 {user.profilePic ? <Image src={user.profilePic} alt="Profile" fill className="object-cover" unoptimized /> : <User className="w-6 h-6 text-cyan-300 mx-auto my-auto" />}
//               </button>

//               {dropdownOpen && (
//                 <div className="absolute right-0 top-12 w-64 bg-black/90 border border-gray-700 rounded-lg shadow-lg p-3">
//                   {!editMode ? (
//                     <>
//                       <div className="px-3 py-2 border-b border-gray-700 text-sm text-gray-300">
//                         <p>{user.name}</p>
...
//                       <a href="/componet/dasboard" className="flex items-center gap-2 px-3 py-2 hover:bg-gray-800 rounded-md text-gray-200">
//                         <LayoutDashboard size={16} /> Dashboard
//                       </a>

//                       <button onClick={() => { setEditMode(true); setTempName(user.name); setTempBio(user.bio || ""); setPreviewUrl(user.profilePic || null); }} className="flex items-center gap-2 px-3 py-2 hover:bg-gray-800 rounded-md text-gray-200 w-full text-left">✏️ Edit Profile</button>

//                       <a href="https://accounts.google.com/signup" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-3 py-2 hover:bg-gray-800 rounded-md text-gray-200 w-full text-left">
//                         <PlusCircle size={16} /> Add Another Account
//                       </a>

//                       <button onClick={handleLogout} className="flex items-center gap-2 px-3 py-2 hover:bg-red-500/20 rounded-md text-red-400 w-full">
//                         <LogOut size={16} /> Logout
//                       </button>
//                     </>
//                   ) : (
//                     <div className="flex flex-col gap-3">
//                       <input type="text" value={tempName} onChange={(e) => setTempName(e.target.value)} className="w-full px-2 py-1 rounded bg-gray-800 text-white text-sm" placeholder="Enter name" />
//                       <textarea value={tempBio} onChange={(e) => setTempBio(e.target.value)} className="w-full px-2 py-1 rounded bg-gray-800 text-white text-sm" placeholder="Enter bio" />
//                       <input type="file" accept="image/*" onChange={(e) => handlePicUpload(e.target.files?.[0] || null)} className="text-xs text-gray-400" />
//                       {previewUrl && <Image src={previewUrl} alt="Preview" width={60} height={60} className="rounded-full mx-auto" unoptimized />}
//                       <div className="flex gap-2">
//                         <button onClick={handleSaveProfile} className="flex-1 px-3 py-1 bg-cyan-600 text-white rounded hover:bg-cyan-700 text-sm">Save</button>
//                         <button onClick={() => setEditMode(false)} className="flex-1 px-3 py-1 bg-gray-700 text-white rounded hover:bg-gray-800 text-sm">Cancel</button>
//                       </div>
//                     </div>
//                   )}
//                 </div>
...
//           ) : (
//             <>
//               <button onClick={handleLogin} className="px-4 py-2 rounded-lg border border-cyan-400 text-cyan-300 hover:bg-cyan-500/20">Login with Gmail</button>
//               <a href="https://accounts.google.com/signup" target="_blank" rel="noopener noreferrer" className="px-4 py-2 rounded-lg bg-gradient-to-r from-cyan-500 to-blue-700 text-white">Sign Up</a>
//             </>
//           )}
//         </div>
