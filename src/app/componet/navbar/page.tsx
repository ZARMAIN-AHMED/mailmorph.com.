...
const Navbar: React.FC = () => {
  ...
  const fetchUser = async () => {
    try {
      const res = await fetch(
        "https://mailmorph-back-xyz-production.up.railway.app/auth/me",
        {
          credentials: "include",
        }
      );
  ...
  // ALWAYS CHECK USER DIRECTLY FROM BACKEND
  useEffect(() => {
    fetchUser();
  }, []);

  // Refresh UI when user changes
  useEffect(() => {}, [user]);
  ...
  const handleLogin = () => {
    const loginWindow = window.open(
      `https://mailmorph-back-xyz-production.up.railway.app/auth/login`,
      "_blank"
    );
  ...
  const handleLogout = async () => {
    try {
      const res = await fetch(
        `https://mailmorph-back-xyz-production.up.railway.app/auth/logout`,
        {
          method: "POST",
          credentials: "include",
        }
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      const res = await fetch(
        `https://mailmorph-back-xyz-production.up.railway.app/user/update`,
        {
          method: "PATCH",
          body: formData,
          credentials: "include",
  ...
  const menu = user
    ? [...baseMenu, { name: "History", href: "/componet/history" }, { name: "Dashboard", href: "/componet/dasboard" }]
    : baseMenu;
    ...
    <nav className="fixed top-0 left-0 w-full z-50 bg-black/40 backdrop-blur-md border-b border-white/10">
      <div className="max-w-7xl mx-auto flex justify-between items-center py-4 px-6">
        ...
        <a href="/" className="flex items-center gap-3">
          <Image src="/logo2.png" alt="logo" width={55} height={55} unoptimized />
          <span className="text-2xl font-extrabold text-white">MailMorph</span>
        </a>
        ...
        <ul className="hidden md:flex gap-8 items-center">
          {menu.map((item) => (
            <li key={item.name}>
              <a href={item.href} className="text-gray-200 hover:text-white">{item.name}</a>
            </li>
          ))}
        </ul>

        {/* USER SECTION */}
        <div className="hidden md:flex gap-4 items-center" ref={dropdownRef}>
          {user ? (
            <>
              <button onClick={() => setDropdownOpen(!dropdownOpen)} className="w-10 h-10 rounded-full overflow-hidden border-2 border-cyan-400">
                {user.profilePic ? (
                  <Image src={user.profilePic} alt="pic" fill unoptimized className="object-cover" />
                ) : (
                  <User className="text-cyan-300 mx-auto mt-2" />
                )}
              </button>

              {dropdownOpen && (
                <div className="absolute right-0 top-12 w-64 bg-black/90 border border-gray-700 rounded-lg p-3">
                  {!editMode ? (
                    <>
                      <p className="text-white font-bold">{user.name}</p>
                      <p className="text-xs text-gray-400 mb-2">{user.email}</p>

                      <a href="/componet/dasboard" className="flex items-center gap-2 text-gray-200 py-2">
                        <LayoutDashboard size={16} /> Dashboard
                      </a>
                      ...
                      <a
                        href="https://accounts.google.com/signup"
                        target="_blank"
                        className="flex items-center gap-2 text-gray-200 py-2"
                      >
                        <PlusCircle size={16} /> Add Another Account
                      </a>

                      <button onClick={handleLogout} className="text-red-400 py-2 flex gap-2">
                        <LogOut size={16} /> Logout
                      </button>
                    </>
                  ) : (
                    <div className="flex flex-col gap-2">
                      ...
                      <textarea
                        ...
                        onChange={(e) => setTempBio(e.target.value)}
                      />
                      <input type="file" onChange={(e) => handlePicUpload(e.target.files?.[0] || null)} />

                      {previewUrl && (
                        <Image src={previewUrl} width={60} height={60} alt="preview" className="rounded-full mx-auto" unoptimized />
                      )}

                      <button onClick={handleSaveProfile} className="bg-cyan-600 text-white py-1 rounded">Save</button>
                      <button onClick={() => setEditMode(false)} className="bg-gray-700 text-white py-1 rounded">Cancel</button>
                    </div>
                  )}
                </div>
              )}
            ...
            <>
              <button onClick={handleLogin} className="border border-cyan-400 px-3 py-1 rounded text-cyan-300">
                Login with Gmail
              </button>
              <a
                href="https://accounts.google.com/signup"
